...
namespace inst::offline
{
    namespace {
        ...
        // Whether ANY legacy icon directory exists at all, checked once and
        // cached. Without this, every title that misses the packed-icon
        // lookup falls through to probing 3 directories x 7 extensions =
        // up to 21 real filesystem stat() calls, every single time, even
        // when none of these legacy folders exist in the first place (the
        // common case for an app that only ever used the packed format).
        // With up to 60 titles per shop page, that's 1000+ SD-card stat
        // calls per page load, which is a real, measurable slowdown.
        bool g_anyLegacyIconDirAttempted = false;
        bool g_anyLegacyIconDirExists = false;
        bool AnyLegacyIconDirExists()
        {
            if (g_anyLegacyIconDirAttempted)
                return g_anyLegacyIconDirExists;
            g_anyLegacyIconDirAttempted = true;
            for (const auto& dir : GetLegacyIconDirectories()) {
                std::error_code ec;
                if (std::filesystem::exists(dir, ec) && std::filesystem::is_directory(dir, ec)) {
                    g_anyLegacyIconDirExists = true;
                    break;
                }
            }
            return g_anyLegacyIconDirExists;
