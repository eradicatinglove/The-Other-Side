...
namespace inst::config {
    ...
    std::string BuildRemoteUrl(const RemoteProfile& remote)
    {
        ...
        // Only include an explicit port when it differs from the protocol's
        // standard default. Many reverse proxies / CDNs (Cloudflare, nginx,
        // Caddy) do exact-match Host header routing and will fail silently
        // (empty response, connection reset, 0 files loaded) if the default
        // port is explicitly appended (e.g. "https://host:443" instead of
        // "https://host").
        bool isDefaultPort = (protocol == "https" && port == 443) ||
    ...
    void parseConfig() {
        ...
        remoteLegacyMode = true;  // Send Version: 20.0 to be compatible with shops like Ghost eShop
