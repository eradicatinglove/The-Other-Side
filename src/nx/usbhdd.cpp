...
namespace nx::hdd
{
    static constexpr u32 kMaxDevices = 32;
    static UEvent* g_statusChangeEvent = nullptr;
    static UEvent g_exitEvent = {0};

    static u32 g_usbDeviceCount = 0;
    static UsbHsFsDevice g_usbDevices[kMaxDevices];
    ...
    static int entry(void* arg)
    {
        ...
        while (true)
        {
            ...
            if (!(listed_device_count = usbHsFsListMountedDevices(g_usbDevices, std::min(g_usbDeviceCount, kMaxDevices))))
            {
                continue;
            }
        }
    ...
    u32 count()
    {
        return std::min(g_usbDeviceCount, kMaxDevices);
    }
    ...
    bool init()
    {
        ...
        thrd_create(&g_thread, entry, nullptr);
